...
The list is filled from `GET /api/places?bbox=west,south,east,north&limit=600`. Capture that response while the map loads. Its `total` is the number in the heading. An empty local database shows `0 places in this area` and the empty copy, and the API returns `"places":[]` with `"total":0`.
